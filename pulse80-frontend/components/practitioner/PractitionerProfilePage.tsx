...
const locations = { Botswana: { districts: ["Central", "Chobe", "Francistown", "Gaborone", "Kweneng", "North-East", "North-West", "South-East", "Southern"], cities: ["Gaborone", "Francistown", "Maun", "Molepolole", "Kasane", "Palapye"] }, "South Africa": { districts: ["Gauteng", "Western Cape", "KwaZulu-Natal", "Eastern Cape", "Free State", "Limpopo", "Mpumalanga", "North West", "Northern Cape"], cities: ["Johannesburg", "Pretoria", "Cape Town", "Durban", "Gqeberha", "Bloemfontein"] } };
...
function MultiSelectField({ label, value, options, editable, onChange }: { label: string; value: string[]; options: string[]; editable: boolean; onChange: (value: string[]) => void }) { if (!editable) return <ReadField label={label} value={value.join(", ")} />; return <label className="block"><span className="text-xs font-semibold text-navy">{label}</span><select multiple value={value} onChange={(event) => onChange(Array.from(event.target.selectedOptions, (option) => option.value))} className="mt-2 min-h-28 w-full rounded-lg border border-primary/40 bg-white px-3 py-2 text-sm">{options.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>; }
function DocumentUploadList({ documents, documentFiles, documentUploadProgress, uploadingDocument, onFileChange, onUpload }: { documents: PractitionerProfile["documents"]; documentFiles: Record<string, File | null>; documentUploadProgress: number | null; uploadingDocument: boolean; onFileChange: (documentType: string, file: File | null) => void; onUpload: () => void }) {
  return <div className="space-y-4">
    <div className="grid gap-4 md:grid-cols-2">{documentTypes.map((documentType) => {
      ...
      return <div key={documentType} className="rounded-lg border border-card-border bg-soft-bg p-4"><p className="text-sm font-semibold text-navy">{documentType}</p>{existingDocument ? <p className="mt-3 truncate text-sm text-subtle" title={existingDocument.fileName}>{existingDocument.fileName}</p> : <><label htmlFor={`practitioner-document-${documentType}`} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); onFileChange(documentType, event.dataTransfer.files[0] ?? null); }} className="mt-3 flex min-h-28 cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed border-primary/40 bg-white px-4 py-4 text-center text-sm text-subtle hover:border-primary hover:bg-primary/5"><CloudUpload className="mb-2 h-7 w-7 text-primary" /><span className="font-semibold text-navy">{file?.name ?? "Drop a document here or choose a file"}</span><span className="mt-1 text-xs">PDF, PNG, or JPEG</span></label><input id={`practitioner-document-${documentType}`} type="file" accept="application/pdf,image/png,image/jpeg" className="sr-only" onChange={(event) => onFileChange(documentType, event.target.files?.[0] ?? null)} /></>}</div>;
    })}</div>
