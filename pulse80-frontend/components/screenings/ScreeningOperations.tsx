...
export function AdminScreeningOperations({ screenings, mode }: { screenings: Screening[]; mode: "screenings" | "results" }) {
  ...
  return <Workspace title={title} description={description} message={message} metrics={[
    { label: mode === "screenings" ? "Records captured" : "Ready for QA", value: mode === "screenings" ? screenings.length : screenings.filter((item) => item.status === "Under Review").length, detail: "Real screening records", icon: Microscope },
    ...
    { label: "Completed", value: screenings.filter((item) => item.status === "Completed").length, detail: "Quality assured", icon: ClipboardCheck },
  ]}><Filters query={query} setQuery={setQuery} risk={risk} setRisk={setRisk} /><Table headers={mode === "screenings" ? ["Reference", "Organisation", "Activation", "Practitioner", "Risk", "Captured", "Status", ""] : ["Reference", "Blood pressure", "Glucose", "Cholesterol", "BMI", "Risk", "QA status", ""]}>{rows.map((item) => <tr key={item.id} className="border-t border-card-border">{mode === "screenings" ? <><Cell><b>{item.participantReference}</b><small>{item.department ?? "Department not set"}</small></Cell><Cell>{item.organisationName}</Cell><Cell>{item.activationName ?? "Assignment capture"}</Cell><Cell>{item.practitionerName}</Cell><Risk item={item} /><Cell>{formatDate(item.capturedAt)}</Cell><Status item={item} /></> : <><Cell><b>{item.participantReference}</b><small>{item.organisationName}</small></Cell><Cell>{measurement(item.result.systolicMmhg, item.result.diastolicMmhg)}</Cell><Cell>{unit(item.result.glucoseMmolL, "mmol/L")}</Cell><Cell>{unit(item.result.cholesterolMmolL, "mmol/L")}</Cell><Cell>{item.result.bmi ?? "—"}</Cell><Risk item={item} /><Status item={item} /></>}<Cell>{item.status === "Under Review" && <button onClick={() => setReviewing(item)} className="font-semibold text-primary">Review</button>}</Cell></tr>)}</Table>{!rows.length && <Empty text="No screening records match these filters." />}{reviewing && <ReviewModal screening={reviewing} pending={pending} onClose={() => setReviewing(null)} onSave={(status, note, errors) => startTransition(async () => { const result = await reviewScreening(reviewing.id, status, note, errors); setMessage(result.ok ? `Screening marked ${status.toLowerCase()}.` : errorText(result.error)); if (result.ok) { setReviewing(null); router.refresh(); } })} />}</Workspace>;
}
...
export function PractitionerScreeningOperations({ screenings, assignments }: { screenings: Screening[]; assignments: ScreeningAssignmentOption[] }) {
  ...
  return <Workspace eyebrow="Health Practitioner" title="Screenings" description="Capture and manage anonymized employee wellness screening records linked to your assigned programmes." message={message} action="Capture screening" onAction={() => setOpen(true)} metrics={[
    ...
    { label: "Under Review", value: submitted, detail: "Awaiting quality assurance", icon: Microscope },
    ...
    <UnifiedFilterCard>
      <div className="grid gap-3 lg:grid-cols-[minmax(240px,1fr)_170px_150px_210px_auto]">
        ...
        <UnifiedFilterSelect label="Status" value={statusFilter} options={["All", "Under Review", "Needs Correction", "Completed"]} onChange={setStatusFilter} />
...
function Status({ item }: { item: Screening }) { return <Cell><StatusBadge status={item.status} tone={item.status === "Completed" ? "success" : item.status === "Needs Correction" ? "danger" : "warning"} /></Cell>; }
...
function ReviewModal({ screening, pending, onClose, onSave }: { screening: Screening; pending: boolean; onClose: () => void; onSave: (status: "Completed" | "Needs Correction", note: string, errors: Array<{ field: string; message: string }>) => void }) { const [status, setStatus] = useState<"Completed" | "Needs Correction">("Completed"); const [note, setNote] = useState(""); const [errorsText, setErrorsText] = useState(""); const errors = errorsText.split("\n").map((line) => { const [field, ...message] = line.split(":"); return { field: field.trim(), message: message.join(":").trim() }; }).filter((item) => item.field && item.message); return <Modal title={`Review ${screening.participantReference}`} pending={pending} valid={status === "Completed" || errors.length > 0} onClose={onClose} onSubmit={() => onSave(status, note, errors)}><Field label="QA decision"><select className={inputClass} value={status} onChange={(event) => setStatus(event.target.value as typeof status)}><option>Completed</option><option>Needs Correction</option></select></Field><Field label="Reviewer note"><input className={inputClass} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Optional overall note" /></Field>{status === "Needs Correction" ? <label className="sm:col-span-2 text-xs font-semibold text-navy">Correction errors (one per line: field: message)<textarea className={`${inputClass} mt-2 min-h-28 py-3`} value={errorsText} onChange={(event) => setErrorsText(event.target.value)} placeholder={"Blood pressure: Result is missing\nConsent: Confirmation is missing"} /></label> : null}<div className="sm:col-span-2 rounded-lg bg-[#f8fafc] p-4 text-sm text-navy">Risk: <b>{screening.result.riskLevel}</b> · Measurements: {measurementSummary(screening)}</div></Modal>; }
