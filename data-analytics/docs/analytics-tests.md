...
Use Python 3.12, as a normal non-root user. The pinned `pgserver` package supplies PostgreSQL binaries. Windows users can run this in WSL. The GitHub workflow runs the same suite on Ubuntu.
...
Record the branch commit, command, PostgreSQL version, pass/fail counts and any discrepancy in the PR.
